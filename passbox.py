...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.frame_left = customtkinter.CTkFrame(master=self, corner_radius=3)
        self.frame_left.grid(row=0, column=0, sticky="nswe", padx=10, pady=10)

        self.frame_right = customtkinter.CTkFrame(master=self, corner_radius=3)
        ...
        self.label_right2 = customtkinter.CTkLabel(
            master=self.frame_right, text="login:"
        )
        self.label_right2.grid(row=5, column=0, pady=0, padx=10, sticky="w")

        self.label_right3 = customtkinter.CTkLabel(
            master=self.frame_right, text="pass:"
        )
    ...
    def get_ico(self):
        try:
            if DEV:
                self.iconbitmap(default=ICOFILE)
                return
            path = os.path.join(sys._MEIPASS, ICOFILE)
    ...
    def load_dict(self):
        """загружает и декодирует словарь"""
        try:
...
if __name__ == "__main__":
    app = App()
    app.load_dict()  # if not exists interface_disable()
    app.mainloop()
